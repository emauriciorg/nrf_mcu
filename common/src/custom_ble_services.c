...
void cs_data_handler_f(ble_cs_t * p_custom_service, uint8_t * p_data, uint16_t length)
{
...
void custom_service_init(ble_cs_t * p_custom_service)
{
	...
	ble_uuid128_t     base_uuid = BLE_UUID_OUR_BASE_UUID;
    	...
    	p_custom_service->data_handler            =cs_data_handler_f;// p_custom_service_init->data_handler;
    	p_custom_service->is_notification_enabled = true;
	...
	err_code = custom_services_add_char_rx(p_custom_service);
	BLE_OUT("rx got error %x\n", err_code);
	APP_ERROR_CHECK(err_code);

	err_code = custom_services_add_char_tx(p_custom_service);
	BLE_OUT("tx got error %x\n", err_code);
	APP_ERROR_CHECK(err_code);


    BLE_OUT( "Executing our_service_init().\n");
    BLE_OUT( "Service UUID: %x\n", service_uuid.uuid);
    BLE_OUT( "Service UUID type: %x\n", service_uuid.type);
    BLE_OUT( "Service handle: %x\n", p_custom_service->service_handle);
...
uint32_t ble_nus_string_send(ble_cs_t * p_custom_service, uint8_t * p_string, uint16_t length)
{
    ...
    {
        BLE_OUT("INVALID CONN HANDLE/NOTIFICIATION DISABLED\n");
        return NRF_ERROR_INVALID_STATE;
    }

    if (length > BLE_CUS_MAX_DATA_LEN)
    {
        BLE_OUT("max lent reached/ data not send\n");

...
extern   st_uart_string uart_stream;
#endif
#define SDBG app_trace_log
...
uint32_t ble_advertising_start(ble_adv_mode_t advertising_mode)
{
    ...
    switch (m_adv_mode_current)
    {
        case BLE_ADV_MODE_DIRECTED:
            SDBG("[ADV]: Starting direct advertisement.\r\n");
        ...
        case BLE_ADV_MODE_FAST:
                ...
                && !m_whitelist_temporarily_disabled)
            {
                ...
                SDBG("[ADV]: Starting fast advertisement with whitelist.\r\n");
            }
            else
            {
                m_adv_evt = BLE_ADV_EVT_FAST;
                SDBG("[ADV]: Starting fast advertisement.\r\n");
            }
            break;
        ...
        case BLE_ADV_MODE_SLOW:
                ...
                && !m_whitelist_temporarily_disabled)
            {
                ...
                SDBG("[ADV]: Starting slow advertisement with whitelist.\r\n");
            }
            else
            {
                m_adv_evt = BLE_ADV_EVT_SLOW;
                SDBG("[ADV]: Starting slow advertisement.\r\n");
            }
            break;
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_TIMEOUT:
            if (p_ble_evt->evt.gap_evt.params.timeout.src == BLE_GAP_TIMEOUT_SRC_ADVERTISING)
            {
                ...
                {
                    case BLE_ADV_MODE_DIRECTED:
                        SDBG("[ADV]: Timed out from directed advertising.\r\n");
                        {
                            uint32_t err_code;
                        ...
                        break;
                    case BLE_ADV_MODE_DIRECTED_SLOW:
                        SDBG("[ADV]: Timed out from directed slow advertising.\r\n");
                        {
                            uint32_t err_code;
                    ...
                    case BLE_ADV_MODE_FAST:
                    {
                        uint32_t err_code;
                        m_adv_evt = BLE_ADV_EVT_FAST;
                        SDBG("[ADV]: Timed out from fast advertising, starting slow advertising.\r\n");
                    ...
                    case BLE_ADV_MODE_SLOW:
                        m_adv_evt = BLE_ADV_EVT_IDLE;
                        SDBG("[ADV]: Timed out from slow advertising, stopping advertising.\r\n");
                        if (m_evt_handler != NULL)
                        {
                            m_evt_handler(m_adv_evt);
                        }
